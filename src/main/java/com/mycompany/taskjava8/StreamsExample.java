...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        Function<Author, List<Book>> booksFunction = new Function<Author, List<Book>>() {
            @Override
        ...
        authors.stream()
               .flatMap(author -> booksFunction.apply(author).stream())
        ...
        // TODO With functional interfaces declared
        ToIntFunction<Book> priceMapper = new ToIntFunction<Book>() {
            @Override
        ...
        // TODO With functional interfaces declared
        Predicate<Author> activeWithPublishedBooks = new Predicate<Author>() {
            @Override
            public boolean test(Author author) {
                return author.active && author.books.stream().anyMatch(publishedBookPredicate);
            }
        };

        authors.stream()
               .filter(activeWithPublishedBooks)
               .forEach(authorPrintConsumer);
